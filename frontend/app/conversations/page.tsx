...
export default function ConversationsPage() {
  ...
  return (
    <div className="flex-1 flex h-[calc(100vh)] overflow-hidden bg-slate-950">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-semibold text-xs text-white truncate">{c.customer.name}</span>
                  ...
                  {isSelected && isAwaitingBotReply ? (
                    <div className="text-[11px] text-emerald-400 italic flex items-center gap-1.5 mb-1">
                      <span>typing...</span>
                      <span className="inline-flex items-center gap-0.5">
                        <span className="w-1 h-1 rounded-full bg-emerald-400 animate-wa-dot-1"></span>
                        <span className="w-1 h-1 rounded-full bg-emerald-400 animate-wa-dot-2"></span>
                        <span className="w-1 h-1 rounded-full bg-emerald-400 animate-wa-dot-3"></span>
                      </span>
                    </div>
                  ) : (
      ...
      {selectedId && activeConv ? (
        <div className="flex-1 flex flex-col bg-slate-950 overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-800 bg-slate-900/60 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-sm text-emerald-400">
                {activeConv.customer.name.slice(0, 2).toUpperCase()}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-sm text-white">{activeConv.customer.name}</h3>
                  <span
                ...
                {isAwaitingBotReply ? (
                  <p className="text-xs text-emerald-400 flex items-center gap-1.5 font-medium italic mt-0.5">
                    <span>typing...</span>
                    <span className="inline-flex items-center gap-0.5">
                      <span className="w-1 h-1 rounded-full bg-emerald-400 animate-wa-dot-1"></span>
                      <span className="w-1 h-1 rounded-full bg-emerald-400 animate-wa-dot-2"></span>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Delete this chat"
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-rose-400 hover:text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 transition-all"
              >
                <Trash2 className="h-3.5 w-3.5" />
                Delete Chat
              </button>
          ...
          <div className="px-4 py-2.5 bg-slate-900/90 border-b border-slate-800/80 flex flex-wrap items-center gap-2 text-xs">
            ...
            <button
              ...
              disabled={simulating}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-emerald-500/20 text-slate-200 hover:text-emerald-300 border border-slate-700 text-[11px] font-medium transition-colors"
            >
            ...
            <button
              ...
              disabled={simulating}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-blue-500/20 text-slate-200 hover:text-blue-300 border border-slate-700 text-[11px] font-medium transition-colors"
            >
            ...
            <button
              ...
              disabled={simulating}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-rose-500/20 text-slate-200 hover:text-rose-300 border border-slate-700 text-[11px] font-medium transition-colors"
            >
            ...
            >
              <input
                ...
                className="bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1 text-[11px] text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 w-44"
              />
              <button
                type="submit"
                disabled={!patientInput.trim() || simulating}
                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-slate-950 font-bold rounded-lg text-[11px] transition-colors"
              >
                Send as Patient
              </button>
            </form>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {loadingChat && messages.length === 0 ? (
              ...
              <div className="p-8 text-center text-xs text-slate-500">No message history</div>
            ) : (
              messages.map((m) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`max-w-md rounded-2xl p-4 text-xs leading-relaxed space-y-1 shadow-md ${
                        isCustomer
                          ? 'bg-slate-900 border border-slate-800 text-slate-100 rounded-tl-sm'
                          : isHumanStaff
                          ? 'bg-blue-950/80 border border-blue-500/40 text-blue-100 rounded-tr-sm'
                          : 'bg-emerald-950/60 border border-emerald-500/30 text-emerald-100 rounded-tr-sm'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-4 text-[10px] font-bold text-slate-400 pb-1 border-b border-slate-800/40">
                        <span>
                          {isCustomer
                            ? 'Patient'
                            : isHumanStaff
            ...
            {isAwaitingBotReply && (
              <div className="flex flex-col items-start transition-all duration-300 animate-in fade-in">
                <div className="bg-slate-900 border border-slate-800 text-slate-300 rounded-2xl rounded-tl-sm px-4 py-3 shadow-md flex items-center gap-2.5">
                  <span className="text-xs font-normal text-slate-300 italic tracking-wide">typing...</span>
                  <div className="flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-wa-dot-1"></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-wa-dot-2"></span>
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-wa-dot-3"></span>
                  </div>
                </div>
              </div>
            )}
          ...
          >
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-slate-950 font-semibold text-xs flex items-center gap-1.5 transition-colors shadow-md shadow-emerald-500/20"
            >
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-500 p-8">
          ...
          <p className="text-xs text-slate-600 mt-1">Live customer chats will appear in real time</p>
        </div>
      )}
    </div>
  );
